...
def train_val_test_split(dataset, val_ratio=0.1, test_ratio=0.1):
    ...
    train_size = int((1 - val_ratio - test_ratio) * len(dataset))
    val_test_size = len(dataset) - train_size
    train_set, val_test_set = random_split(dataset, [train_size, val_test_size])
    test_size = int(test_ratio * len(dataset))
    val_size = len(val_test_set) - test_size
